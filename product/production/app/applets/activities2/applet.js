define(
		[ 'backbone', 'marionette', 'moment', 'jquery', 'handlebars',
				'backgrid', 'app/applets/activities2/toolbar/toolbarView',
				'app/applets/activities2/utils',
				'app/applets/activities2/config',
				'app/applets/task_forms/activities/order.consult/utils' ],
		function(Backbone, Marionette, moment, $, Handlebars, Backgrid,
				ToolbarView, Util, Config, ConsultUtils) {
			'use strict';
			...
			var body = Backgrid.Body.extend({
				makeComparator : function(attr, order, func) {
					return function(left, right) {
						var l = func(left, attr, order), r = func(right, attr,
								order), t;
			...
			var AppletLayoutView = ADK.AppletViews.GridView
					.extend({
						...
						initialize : function(options) {
							...
							if (this.options.appletConfig.fullScreen) {
								this.parentWorkspace = ADK.Messaging
										.request('get:current:workspace');
								var expandedModel = ADK.SessionStorage.get
										.sessionModel('expandedAppletId');
								if (!_.isUndefined(expandedModel)
										&& !_.isUndefined(expandedModel
												.get('id'))) {
									this.expandedAppletId = expandedModel
											.get('id');
								}
							}
							var dataGridOptions = {
							...
							var filter = ADK.SessionStorage
									.getAppletStorageModel(
											this.expandedAppletId, 'filter',
											true, this.parentWorkspace);
							...
							this.bindEntityEvents(this.sharedModel,
									this.sharedModelEvents);
							...
							var config = Config.getColumnnsAndFilterFields(
									this.columnsViewType, this.contextViewType);
							...
							if (this.columnsViewType === 'expanded') {
								...
								// Default date picker options when the screen
								// is loaded
								var fromDate, toDate;
								var globalDate = ADK.SessionStorage
										.getModel('globalDate');
								if (!this.fullScreen
										&& globalDate.get('selectedId') !== undefined
										&& globalDate.get('selectedId') !== null) {
									fromDate = moment(
											globalDate.get('fromDate'),
											'MM/DD/YYYY').format('YYYYMMDD');

									if (fromDate.length === 8) {
										fromDate += '0000';
									}

									toDate = moment(globalDate.get('toDate'),
											'MM/DD/YYYY').format('YYYYMMDD');
									...
									toDate = moment().add('months', 6).format(
											'YYYYMMDD')
											+ '2359';
								...
								this.listenTo(ADK.Messaging,
										'globalDate:selected', function(
												dateModel) {
											self.dateRangeRefresh('createdOn',
													dateModel.toJSON());
										});
							}

							if (this.contextViewType === 'patient'
									&& ADK.PatientRecordService
											.isPatientInPrimaryVista()) {
								dataGridOptions.onClickAdd = function() {
									var TrayView = ADK.Messaging
											.request("tray:writeback:actions:trayView");
									if (TrayView) {
							...
							this.listenTo(ADK.Messaging
									.getChannel('activities2'),
									'create:success', this.refresh);
							this.setFetchOptions();
							this.activitiesCollection = ADK.ResourceService
									.fetchCollection(this.fetchOptions);
							dataGridOptions.collection = this.activitiesCollection;
							this.bindEntityEvents(this.activitiesCollection,
						...
						setFetchOptions : function() {
							...
							if (secondaryFilter === 'open'
									|| secondaryFilter === 'closed') {
							...
							if (secondaryFilter !== 'open'
									&& this.sharedModel.has('toDate')
									&& this.sharedModel.has('fromDate')) {
								var endDate = moment(this.sharedModel
										.get('toDate'), 'YYYYMMDDHHmm');
								var startDate = moment(this.sharedModel
										.get('fromDate'), 'YYYYMMDDHHmm');
								var utcEndDate = moment.utc(endDate).format(
										'YYYYMMDDHHmm');
							...
							if (this.contextViewType === 'patient') {
								this.fetchOptions.criteria.pid = ADK.PatientRecordService
										.getCurrentPatient().get('pid');
							}
						...
						changeFilter : function() {
							ADK.SessionStorage.setAppletStorageModel(
									this.expandedAppletId, 'filter',
									this.sharedModel.get('filter'), true,
						...
						disableFilters : function(disabled) {
							var appletId = this.fullScreen ? 'activities2'
									: this.expandedAppletId;
							this.$el.find(
									'#' + appletId + '-primary-filter-options')
									.prop('disabled', disabled);
							this.$el.find(
									'#' + appletId + '-display-only-options')
									.prop('disabled', disabled);
						},
						dateRangeRefresh : function(dateFieldName,
								filterOptions) {
							var toDate = moment(filterOptions.toDate,
									'MM/DD/YYYY').format('YYYYMMDD');
							var fromDate = moment(filterOptions.fromDate,
									'MM/DD/YYYY').format('YYYYMMDD');
						...
						onDestroy : function() {
							this.unbindEntityEvents(this.sharedModel,
									this.sharedModelEvents);
							this.unbindEntityEvents(this.activitiesCollection,
									this.collectionEvents);
						}
					});
